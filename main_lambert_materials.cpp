...
color ray_color(
  ...
  if (scene.hit(r, 0.001, infinity, hit)) {
    ...
    ray bounce_ray;
    hit.material->scatter(r, hit, attenuation, bounce_ray);
...
int main() {
  ...
  hittable_list scene;
  color albedo1(0.5, 0.5, 0.5);
  shared_ptr<lambertian> mat1 = make_shared<lambertian>(albedo1);
  color albedo2(0.2, 0.2, 0.2);
  shared_ptr<lambertian> mat2 = make_shared<lambertian>(albedo2);
  scene.add(
    make_shared<sphere>(
      point3(0, 0, -1),
      0.5,
    ...
    for (int i = 0; i < image_width; ++i) {
      ...
      for (int s = 0; s < samples_per_pixel; s++) {
        ...
        pixel_color += ray_color(
          r, scene, bg_color_1, bg_color_2, max_bounces, true
        );
      }
